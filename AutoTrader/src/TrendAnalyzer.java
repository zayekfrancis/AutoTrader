...
public class TrendAnalyzer {
	...
	/*
	 * 1) Check if price is below the SMA(50) 
	 * 2) Check if SMA(50) has started to trend downwards 
	 * 3) Check ADX > 20 
	 * 4) Check if difference of +DI and -DI > -10 
	 * 	  and was the change/cross recent 
	 * 5) SAR is above current price
	 */
	public static boolean isNegativelyTrending(Stock stock, String date) {
		// 1) Check if price is below the SMA(50)
		boolean priceAboveSMA = false;
		double currentPrice = stock.getClose(date);
		double sma = stock.getFiftySMA(date);
		...
		// 2) Check if SMA(50) has started to trend downwards
		boolean movingDownwards = isSmaMovingDownwards(stock, date);

		// 3) Check ADX > 20
		// 4) Check if difference of +DI and -DI < -10 
		//	  and was the change/cross recent
		boolean adxAndDISellSignal = ADXAnalyzer.sellSignal(stock, stock.getTimePeriod(), date);

		// 5) SAR is above current price
		boolean SARSignal = currentPrice < stock.getSAR(date) && SARAnalyzer.earlyNegativeReversal(stock, date);
		return (priceAboveSMA && movingDownwards && adxAndDISellSignal && SARSignal);
	}
	...
	/*
	 * 1) If 75% of the early prices were above the moving average 
	 * 2) If 80% of the the newer prices are above the max moving average
	 */
	private static boolean isSmaMovingDownwards(Stock stock, String date) {
		int timePeriod = stock.getTimePeriod();
		int endingIndex = stock.getDates().indexOf(date) - 14;
		int startingIndex = endingIndex - 49;
		if (startingIndex < 0) {
		...
		double max = 0;
		try {
			max = stock.getSMA(stock.getDates().get(startingIndex));
		} catch (ArrayIndexOutOfBoundsException e) {
			e.printStackTrace();
			return false;
		}
