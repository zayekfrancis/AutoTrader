...
public class SARAnalyzer {
	...
	public static boolean earlyReversal(Stock stock, String date) {
		int startingIndex = stock.getDates().indexOf(date) - 3;
	...
	public static boolean calculateSAR(Stock stock, int timePeriod, double baseAF, double maxAF) {
		timePeriod = 3;
		...
		for (int i = firstDateIndex + 1; i <= stock.getDates().size(); i++) {
			...
			if (reversal || low > previousSAR) {
				...
				if (extremePoint > previousEP) {
					accelerationFactor = ((accelerationFactor + baseAF) >= maxAF) ? maxAF : (accelerationFactor + baseAF);
				}
				lastPeriodRange++;
				continue;
			}
